...
('trial_verification', 'Confirm Your TGG Club 30-Day Trial', '<h2>Hello, {display_name}!</h2><p>Thanks for requesting a 30-day Trial membership with TGG Club. This is a one-time offer, so please confirm your email address to activate it.</p><p>Click the link below to activate your Trial membership:</p><p><a href=\"{verify_link}\">{verify_link}</a></p><p>This link will expire in <strong>{expires_in}</strong>. Your trial has not started yet, and your 30 days will not begin counting down until you click the link.</p><p>If you did not request this, you can safely ignore this email.</p><p>Best regards,<br>TGG Club Team</p>', 'Sent when a new member registers for the one-time Trial membership, asking them to verify their email before the trial activates.'),
('trial_activated', 'Your 30-Day Trial Membership is Active!', '<h2>Hello, {display_name}!</h2><p>Your email has been verified and your 30-day Trial membership is now active.</p><p><strong>Membership Details:</strong></p><ul><li><strong>Start Date:</strong> {start_date}</li><li><strong>Expiration Date:</strong> {end_date}</li></ul><p>This Trial membership is a one-time offer and cannot be renewed. To continue your membership after it expires, you will need to sign up for a paid membership level.</p><p>You can log in to your account dashboard at: <a href=\"{login_url}\">{login_url}</a></p><p>Best regards,<br>TGG Club Team</p>', 'Sent when a member confirms their email and their one-time Trial membership is activated.')
...
-- Holds the pending email verification for a contact's one-time free Trial signup.
-- The Trial does not activate (subscription is not created) until the link is clicked.
CREATE TABLE IF NOT EXISTS `tgg_trial_verifications` (
  `contact_id` INT NOT NULL,
  `plan_id` INT NOT NULL,
  `token` VARCHAR(64) NOT NULL,
  `expires_at` DATETIME NOT NULL,
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`contact_id`),
  KEY `idx_trial_verification_token` (`token`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
